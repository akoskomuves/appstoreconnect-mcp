...
**App Store versions (2 tools, read-only)** — `src/domains/appstore-versions.ts`. `asc_list_app_store_versions` (per-app or team-wide, optional `platform` filter, default `sort=-createdDate` for newest first) and `asc_get_app_store_version` (expands app + appStoreVersionLocalizations + build). Lookup-only surface so callers can find a version ID before localizing it; version creation, submission, and lifecycle management are deferred (Apple's flows are heavy enough to deserve their own domain).
