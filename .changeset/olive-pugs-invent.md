...
When `territoryId` is set, the fetch ceiling is raised to scan the whole collection regardless of `maxItems` — `maxItems` caps what comes *back*, not how far to look. Apple returns territories in its own order, so capping the fetch at a small `maxItems` could stop before the requested territory and report it as having no entry at all: a silently wrong answer, and reachable precisely because narrowing to one market is when a caller lowers `maxItems`. If a scan is still truncated and nothing matched, the result now says `INCOMPLETE SCAN` rather than showing an empty table.
