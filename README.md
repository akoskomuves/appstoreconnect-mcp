...
| **v0.10.1** | App Store Version write surface: create / patch / delete + submission for review + the state-machine reads. Closes the loop with v0.10 — you can now ship a new version end-to-end through the MCP (build → localize → submit) without opening ASC. | "Translate release notes into 35 locales and submit version 2.5 for review." |
| **v0.11** | Customer reviews (read · respond · filter by sentiment/version) + customer review summarizations (Apple's AI-aggregated review insight, the new resource on the App Store Connect side). | "Draft a response to every 1-star review on the latest version that mentions the export bug. Show me before posting." |
| **v0.12** | App Info (subtitle, primary/secondary category, age rating, primaryLocale) + App Tags + Search Keywords (Apple's new structured-ASO surface, separate from per-version keywords). | "Swap your app's secondary category from Travel to Sports across all platforms, and add 5 new fishing-related search keywords." |
| **v0.13** | Screenshots + App Previews (asset upload per locale × device-class) + Custom Product Pages (channel-specific copy variants, reuses v0.10's localization shape). | "Generate App Store screenshots for 12 locales from these source files, push them to product page variant `paid-ads-summer`." |
| **v0.14** | In-App Events (live promotional events visible in the App Store product page) + Promoted Purchases (in-store IAP promotion surface). | "Create an in-app event 'Salmon Season Opens' for your app running 2026-06-15 to 2026-07-15." |
| **v0.15** | App Availability v2 (territory enable/disable, soft-launch flows) + Phased Release (staged rollout 1%→100%) + App Encryption Declarations (export compliance per build). | "Enable your app in 3 new territories and start a phased release on version 2.5." |
| **v0.16** | TestFlight follow-ons: Beta Feedback Submissions (tester screenshots + crashes) + Build Beta Notifications + Beta Recruitment Criteria (auto-recruit via public link). | "Summarize beta feedback on build 132 by frequency and severity, draft a triage list." |
